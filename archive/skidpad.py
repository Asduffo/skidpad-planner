...
from skidpad_helpers import generate_data, rotate_cones, get_rotational_matrix, translate_cones, random_translation, random_rotation
import time
import yaml
...
class SkidpadPlanner:
    ...
    def calc_yaw_angle(self):
        ...
        for number_of_steps in [self.circle_length, self.circle_length]:
            circle_heading = np.subtract(self.skidpad_path[current_path_point + 1], self.skidpad_path[current_path_point])
            yaw_angle_list = np.append(yaw_angle_list, self.get_angle(heading_before_next_step, circle_heading))
            heading_first_part = np.subtract(self.skidpad_path[current_path_point + 5], self.skidpad_path[current_path_point + 4]) # since yaw is constant in the circle we can take any
            heading_second_part = np.subtract(self.skidpad_path[current_path_point + 6], self.skidpad_path[current_path_point + 5])
            yaw_angle = self.get_angle(heading_first_part, heading_second_part)
            inter_yaw_list = np.repeat(yaw_angle, 2 * self.circle_length - 1) #each circle must be considered two times
            ...
            heading_before_next_step = np.subtract(self.skidpad_path[current_path_point - 2], self.skidpad_path[current_path_point - 1])
            #print(yaw_angle_list.shape)

        # get the yaw_angle for the last part until the car stops
        heading_after_last_circle = np.subtract(self.skidpad_path[current_path_point], self.skidpad_path[current_path_point - 1])
        ...
        if yaw_angle_list.shape[0] != self.skidpad_path.shape[0]:
            raise ValueError("Yaw Angle List ({} Points) and Skidpad Path ({} Points) doesn't match".format(yaw_angle_list.shape[0], self.skidpad_path.shape[0]))
    ...
    def check_estimate(self, radius, fit_error):
       ...
       if radius > self.args["max_radius"] or radius < self.args["min_radius"]:
           return False
       elif fit_error > self.args["max_fit_error"]:
           return False
       else:
           return True
    ...
    def fit_circles(self, cone_data):
        ...
        filtered_x, filtered_y, filtered_r, fitting_error = least_squares_circle(left_half_filtered)  # run actucal fitting

        return filtered_x, filtered_y, filtered_r, fitting_error

    def run_skidpad(self, noise_range, unsorted_cones, car_orientation, starting_point, translation, center_skidpad, end_point):
        '''
        ...
        if self.simulate_range:
            self.cone_positions = self.sort_by_distance(self.cone_positions, starting_point, 1000)
        ...
        left_side, right_side = self.check_cones(starting_point, left_side, right_side, self.args["rejection_margin"], skidpad_angle, translation)
        ...
        upper_left, lower_left = self.get_circle_angle_sampling(center_left, left_r, self.args["circle_discretization"])
        upper_right, lower_right =  self.get_circle_angle_sampling(center_right, right_r, self.args["circle_discretization"])

        # compute the enter and the finish line that are required to reach the circles and leave the skidpad course
        beginning = self.sample_beginning_line(starting_point, self.args["beginning_discretization"], np.vstack((upper_right, lower_right)),center_skidpad)
        finish_line = self.sample_end_path(end_point, self.args["end_discretization"], np.vstack((upper_left, lower_left)), center_skidpad)
        ...
        self.skidpad_path = self.get_skidpad_path(beginning, np.vstack((upper_left, lower_left)), np.vstack((upper_right, lower_right)), finish_line)
...
if __name__ == "__main__":
 # define real data with some rotation w.r.t to global coordinate frames
    ...
    buggy_perception = args["buggy_perception"] # percentage of cones that are thrown out randomly to test robustness
    ...
    left_real, right_real = generate_data(rotation_angle, translation, buggy_perception) #get testing data for development purposes
    ...
    for noise in noise_list:
        ...
        for k in range(num_iterations):
            start = time.time()
            upper_left, lower_left, upper_right, lower_right, beginning, finish_line, entire_path = skidpad_planner.run_skidpad(noise, unsorted_data, car_orientation, starting_point, translation, center_skidpad, ending_point)
            end = time.time()
            ...
            # plt.scatter(entire_path[:,0], entire_path[:,1])
